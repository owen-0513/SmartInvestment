    ...
    if '/' in symbol:  # 檢測加密貨幣對
        ...
        try:
            ...
            if not ohlcv:
                ...
                print(f"獲取到 {symbol} 的 OHLCV 資料: {ohlcv[:5]}...")  # 打印前5行資料進行調試
...
def create_lstm_model(data, time_steps=80):
    ...
    if len(train_data) < time_steps:
        raise ValueError(f"訓練資料不足: 只有 {len(train_data)} 行，但需要至少 {time_steps} 行。")
    ...
    if len(X_train.shape) == 3:  # 確保形狀為 (樣本數, 時間步長, 特徵數)
        X_train = X_train.reshape(X_train.shape[0], X_train.shape[1], X_train.shape[2])  # 保留特徵數
    ...
    model = Sequential()
    model.add(LSTM(50, return_sequences=True, input_shape=(X_train.shape[1], X_train.shape[2])))  # 更新這裡的 input_shape
    model.add(LSTM(50))
    model.add(Dense(1))
...
def predict():
    try:
        ...
        X_test = np.array([test_data] * 50).reshape((50, test_data.shape[0], test_data.shape[1]))

        predictions = model.predict(X_test).flatten()
        
        predictions_scaled = [float(scaler.data_min_[0] + (scaler.data_max_[0] - scaler.data_min_[0]) * pred) for pred in predictions]
        ...
        return jsonify({'symbol': symbol, 'prediction': average_prediction, 'trend': trend})
    except Exception as e:
        print(f"預測過程中出錯: {e}")
